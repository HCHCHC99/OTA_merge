...
#line 10 "..\\..\\Adp\\can_module.c"
#line 11 "..\\..\\Adp\\can_module.c"
...
static can_tx_callback_t m_pfnTxCallback = 0;

 
static volatile _Bool m_bTxBusy = 0;

 
static can_rx_cache_t *m_pRxCache = 0;
...
static void can_cfg_bdr_init(can_bdr_t can_bdr, const can_bittime_t *bit_time, 
                              stc_can_bit_time_config_t *stcBitCfg)
{
    if (stcBitCfg == 0) return;

    if (CAN_BDR_CUSTOM == can_bdr) {
        if (bit_time == 0) return;
...
static void can_cfg_init(CM_CAN_TypeDef *CANx, const can_cfg_t *cfg)
{
    ...
    stcCanInit.u16FilterSelect = ((0x01U));
                         ...
                         const can_int_t *can_int)
{
    ...
    if (CANx == ((CM_CAN_TypeDef *)(0x40070400UL))) {
        ...
        __NVIC_ClearPendingIRQ(can_int->can_int_irqn);
        __NVIC_SetPriority(can_int->can_int_irqn, can_int->can_int_pri);
        __NVIC_EnableIRQ(can_int->can_int_irqn);
        
        CAN_IntCmd(CANx, ((1UL << 1U) | (1UL << 2U) | (1UL << 3U) | (1UL << 4U) | (1UL << 5U) | (1UL << 6U) | (1UL << 7U) | (1UL << 9U) | (1UL << 11U) | (1UL << 13U)), DISABLE);
...
int8_t can_transmit_std(uint32_t id, uint8_t *pData, uint8_t Len)
{
    ...
    CAN_FillTxFrame(((CM_CAN_TypeDef *)(0x40070400UL)), (0U), &tx);
...
int8_t can_rx_cache_put(can_rx_cache_t *pstcCache, const stc_can_rx_frame_t *pstcFrame)
{
    if (pstcCache == 0 || pstcFrame == 0) {
        return CAN_RET_ERR_PARAM;
    }
    
    pstcCache->rx_frame[pstcCache->write_idx] = *pstcFrame;
    pstcCache->write_idx = (pstcCache->write_idx + 1) % (5);
    
    if (pstcCache->cnt < (5)) {
        pstcCache->cnt++;
    } else {
        pstcCache->read_idx = (pstcCache->read_idx + 1) % (5);
    }
...
void can_receive_poll(can_rx_cache_t *rx)
{
    ...
    while (CAN_GetRxFrame(((CM_CAN_TypeDef *)(0x40070400UL)), &frame) == (0)) {
...
void can_module_irq_handler(void)
{
    ...
    if ((u32Status & (1UL << 23U)) != 0U) {
        while (CAN_GetRxFrame(((CM_CAN_TypeDef *)(0x40070400UL)), &stcRxFrame) == (0)) {
            if (m_pRxCache != 0) {
                can_rx_cache_put(m_pRxCache, &stcRxFrame);
            }
        }
    }
