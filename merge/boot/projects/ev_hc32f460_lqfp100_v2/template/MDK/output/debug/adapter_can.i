...
#line 9 "..\\..\\Adp\\Adapter_Can.c"
#line 10 "..\\..\\Adp\\Adapter_Can.c"
...
#line 13 "..\\..\\Adp\\Adapter_Can.c"
#line 14 "..\\..\\Adp\\Adapter_Can.c"
...
static void CanIf_TxCompleteCallback(void)
{
    ...
    if (send_ret != CAN_RET_OK) {
        ...
        m_u8TxTail = (m_u8TxTail - 1U) & ((32U) - 1U);
        SEGGER_RTT_printf(LOG_CH_MAIN, "\033[33m" "[%s] " "TX callback: send failed, frame returned to queue" "\033[0m" "\r\n", "CANIF");
    }
}
...
static void CanIf_DispatchRx(const CanMsg_t *pMsg)
{
    ...
    for (i = 0U; i < m_u8RxFilterCount; i++) {
        if (CanIf_MatchFilter(pMsg, &m_astcRxFilters[i])) {
            if (m_astcRxFilters[i].pfnCallback != 0) {
...
static void CanIf_CheckBusOff(void)
{
    ...
    if ((u32Status & (1UL << 0U)) != 0U) {
        if (!m_bBusOff) {
            SEGGER_RTT_printf(LOG_CH_MAIN, "\033[33m" "[%s] " "Bus-Off detected! Starting recovery timer..." "\033[0m" "\r\n", "CANIF");
            m_bBusOff = 1;
            nbDelay_Start(&m_stcBusOffTimer);
        }
        
        if (nbDelay_IsComplete(&m_stcBusOffTimer)) {
            SEGGER_RTT_printf(LOG_CH_MAIN, "\033[32m" "[%s] " "Bus-Off recovery: exiting local reset" "\033[0m" "\r\n", "CANIF");
            CAN_ExitLocalReset((((CM_CAN_TypeDef *)(0x40070400UL))));
            m_bBusOff = 0;
        }
    } else {
        if (m_bBusOff) {
            SEGGER_RTT_printf(LOG_CH_MAIN, "\033[32m" "[%s] " "Bus-Off recovered" "\033[0m" "\r\n", "CANIF");
            m_bBusOff = 0;
        }
    }
}
...
void CanIf_Init(void)
{
    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[32m" "[%s] " "=== CanIf Init Start ===" "\033[0m" "\r\n", "CANIF");
    
     
    static can_cfg_t stcCanCfg = {
        .can_ins = CAN1,
        .CANx = ((CM_CAN_TypeDef *)(0x40070400UL)),
        .en_can_tx = CAN_FUNC_ENABLE,
        .en_can_rx = CAN_FUNC_ENABLE,
        
         
        .gpio_rx = {(0x01U), (0x4000U), (51U)},
        .gpio_tx = {(0x01U), (0x8000U), (50U)},
        ...
        .can_tx_cfg = {
            .en_ptb_single_shot = ((0x10U)),
            .en_stb_single_shot = (0x0U),
            .en_stb_prio_md = (0x0U),
        },
        ...
        .can_int_type = ((1UL << 7U) | (1UL << 3U) | (1UL << 6U) | 
                         (1UL << 5U) | (1UL << 4U) | (1UL << 1U)),
        .can_int = {
    ...
    if (can_module_init(&m_stcCanHandle, &stcCanCfg) != CAN_RET_OK) {
        SEGGER_RTT_printf(LOG_CH_MAIN, "\033[31m" "[%s] " "can_module_init failed!" "\033[0m" "\r\n", "CANIF");
        return;
    }
    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "can_module initialized" "\033[0m" "\r\n", "CANIF");
    
     
    can_register_tx_callback(&CanIf_TxCompleteCallback);
    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "TX callback registered" "\033[0m" "\r\n", "CANIF");
    ...
    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[32m" "[%s] " "=== CanIf Init Done ===" "\033[0m" "\r\n", "CANIF");
}
...
_Bool CanIf_Send(const CanMsg_t *pMsg)
{
    ...
    if (!m_bInitialized || pMsg == 0) {
        return 0;
    }
    
    if (pMsg->u8DLC > 8U) {
        SEGGER_RTT_printf(LOG_CH_MAIN, "\033[31m" "[%s] " "Invalid DLC: %d" "\033[0m" "\r\n", "CANIF",pMsg->u8DLC);
        return 0;
    }
    ...
    if (!can_is_tx_busy()) {
        ...
        if (send_ret == CAN_RET_OK) {
            __enable_irq();
            SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "TX direct: ID=0x%08X, len=%d" "\033[0m" "\r\n", "CANIF",stcFrame . id, stcFrame . len);
            return 1;
        }
    }
     ...
     
    {
        ...
        if (u8Next != m_u8TxTail) {
            ...
            m_u8TxHead = u8Next;
            bResult = 1;
            SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "TX queued: ID=0x%08X, queue=%d" "\033[0m" "\r\n", "CANIF",pMsg->u32ID, (m_u8TxHead - m_u8TxTail) & ((32U) - 1U));

        } else {
            SEGGER_RTT_printf(LOG_CH_MAIN, "\033[33m" "[%s] " "TX queue full! ID=0x%08X dropped" "\033[0m" "\r\n", "CANIF",pMsg->u32ID);
        }
    }
...
_Bool CanIf_RegisterRxFilter(const CanIf_RxFilterEntry_t *pEntry)
{
    if (!m_bInitialized || pEntry == 0) {
        return 0;
    }
    
    if (pEntry->pfnCallback == 0) {
        SEGGER_RTT_printf(LOG_CH_MAIN, "\033[33m" "[%s] " "Register filter: callback is NULL" "\033[0m" "\r\n", "CANIF");
        return 0;
    }
    
    if (m_u8RxFilterCount >= (16U)) {
        SEGGER_RTT_printf(LOG_CH_MAIN, "\033[33m" "[%s] " "Register filter: table full (max=%d)" "\033[0m" "\r\n", "CANIF",(16U));
        return 0;
    }
    ...
    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "Filter registered: ID=0x%08X, Mask=0x%08X, count=%d" "\033[0m" "\r\n", "CANIF",pEntry->u32CanId, pEntry->u32CanMask, m_u8RxFilterCount);
...
void CanIf_SetDefaultRxCallback(void (*pfnCallback)(const CanMsg_t *pMsg))
{
    ...
    SEGGER_RTT_printf(LOG_CH_MAIN, "\033[36m" "[%s] " "Default callback set: %s" "\033[0m" "\r\n", "CANIF",pfnCallback ? "YES" : "NULL");
}
